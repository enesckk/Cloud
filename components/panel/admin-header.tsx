...
const adminPageTitles: Record<string, string> = {
  ...
  "/dashboard/admin/education": "Education Modules",
  "/dashboard/admin/providers": "Providers",
}
...
export function AdminHeader({ onToggleSidebar }: AdminHeaderProps) {
  ...
  const storageKey = getNotificationStorageKey(user?.id, true)
  ...
  // Listen for custom notification events (only for admin panel)
  useEffect(() => {
    if (typeof window === "undefined" || !user?.id || !user?.is_admin) return

    const handleNotification = (e: CustomEvent) => {
      // Only handle admin panel notifications
      if (e.detail.target === "user") return
  ...
  const getPageTitle = () => {
    ...
    // Find partial match
    for (const [path, title] of Object.entries(adminPageTitles)) {
