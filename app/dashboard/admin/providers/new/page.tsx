...
import { createAdminProvider, type Provider } from "@/lib/api-client"
...
export default function NewProviderPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true)
    try {
      await createAdminProvider(user.id, {
        ...formData,
        features: features,
      })
      const description = `"${formData.display_name}" has been added and is now available for cost analysis.`
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent("cloudguide:notification", {
          detail: { type: "success", title: "Provider created successfully", description, target: "admin" },
        }))
      }
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-3xl font-bold">Create New Provider</h1>
        <p className="text-muted-foreground mt-2">Add a new cloud provider to the system</p>
      </div>
